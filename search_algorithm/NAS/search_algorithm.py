...
import utils
from parallel import ParallelOperater, \
    ParallelConfig
from search_space.search_space_config import SearchSpace  # for unit test
...
class Search(object):
    ...
    def __init__(self,
        ...
        self.data_name = gnn_parameter["data_cnf"]
        # parallel estimation operator initialize
        self.parallel_estimation = ParallelOperater(gnn_parameter)
    ...
    def search_operator(self):
        ...
        population_initialization = PopulationInitialization(int(self.search_parameter["initial_num"]),
                                                             self.search_space)
        ...
        # initial gnn architecture time cost record#####创建在文件初始目录在，比如说这个应当创建在my_model下
        ##在这个程序中应当都为26
        #path = os.path.split(os.path.realpath(__file__))[0][:-25] + "logger/graphpas_logger/"
        path="/root/deepfri/logger/graphpas_logger/"
        print(path)
        if not os.path.exists(path):
            os.makedirs(path)
        utils.experiment_time_save_initial(path, self.data_name + "_initial_time.txt", time_initial)
        ...
        for i in range(int(self.search_parameter["search_epoch"])):
            ...
            utils.experiment_graphpas_data_save(path,
                                                self.data_name + "_search_epoch_" + str(i + 1) + ".txt",
                                                sharing_population,
                                                sharing_performance,
                                                self.search_space.space_getter(),
                                                self.search_space.stack_gcn_architecture)
        ...
        utils.experiment_time_save(path,
                                   self.data_name + "_search_time.txt",
                                   epoch,
                                   time_search_list)
if __name__ == "__main__":
    ...
    search_parameter = {"parallel_num": "2",
                        ...
                        "sharing_num": "3",
                        "search_epoch": "7"}
    gnn_parameter = {"data_cnf": 'cc',
                     "gpu_number": "0",
                     "epoch_number": "8",
                     "pre_name": "mf_model"
                     }
    search_space = SearchSpace()
    graphpas_instance = Search(search_parameter, gnn_parameter, search_space)
